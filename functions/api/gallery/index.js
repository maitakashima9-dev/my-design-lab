...
export async function onRequestGet({ request, env }) {
  ...
  const items = results.map(g => ({
    id: g.id, title: g.title, tag: g.tag, thumb: g.thumb_key ? `/api/files/${g.thumb_key}` : null,
  }));
...
export async function onRequestPost({ request, env }) {
  ...
  const result = await env.DB.prepare(
    `INSERT INTO gallery_items (title, tag, thumb_key, sort_order) VALUES (?, ?, ?, ?)`
  ).bind(title, tag, thumbKey || null, countRow.c).run();
